...
class TokenListTest
{
    ...
    void testBasicTokenization()
    {
        ...
        // Start token
        token = iterator.next();
        assertInstanceOf(Token.Start.class, token);

        // 'if' keyword
        token = iterator.next();
        assertInstanceOf(Token.Keyword.class, token);
        assertEquals("if", ((Token.Keyword) token).value());

        // 'count' identifier
        token = iterator.next();
        assertInstanceOf(Token.Identifier.class, token);
        assertEquals("count", ((Token.Identifier) token).value());

        // '>' operator
        token = iterator.next();
        assertInstanceOf(Token.Operator.class, token);
